$(document).ready(function () {
    //#region Phiếu Thu
    let ID = $('#form-phieu-thu input[name="pt-IDThu"]');
    let TiGia = $('#form-phieu-thu input[name="pt-TiGia"]');
    let SoPhieu = $('#form-phieu-thu input[name="pt-SoPhieu"]');
    SoPhieu.prop('disabled', true);
    let Ngay = $('#form-phieu-thu input[name="pt-Ngay"]');
    let CaID = $('#form-phieu-thu select[name="pt-CaID"]');
    let NguoiThu = $('#form-phieu-thu select[name="slNhanVien"]');
    let LoaiTien = $('#form-phieu-thu select[name="pt-LoaiTien"]');
    let SRID = $('#form-phieu-thu select[name="pt-SRID"]');
    SRID.prop('disabled', true);
    let ValueLoaiTien = $('#form-phieu-thu input[name="pt-ValueLoaiTien"]');

    let SoTien = $('#form-phieu-thu input[name="pt-SoTien"]');
    let SoTienVND = $('#form-phieu-thu input[name="pt-SoTienVND"]');
    let Thue = $('#form-phieu-thu input[name="pt-Thue"]');
    let NHID = $('#form-phieu-thu select[name="pt-NHID"]');

    $('#radio-ngan-hang').css('display', 'none');
    let nop = $('input[name="ngan-hang"][value="1"]');
    let rut = $('input[name="ngan-hang"][value="0"]');

    let LyDo = $('#form-phieu-thu select[name="pt-LyDo"]');

    let IDNguoiNhan = $('#form-phieu-thu input[name="pt-IDNguoiNhan"]');
    let CodeNguoiNhan = $('#form-phieu-thu input[name="pt-CodeNguoiNhan"]');
    let NameNguoiNhan = $('#form-phieu-thu input[name="pt-NameNguoiNhan"]');
    let AddressNguoiNhan = $('#form-phieu-thu input[name="pt-AddressNguoiNhan"]');
    let PhoneNguoiNhan = $('#form-phieu-thu input[name="pt-PhoneNguoiNhan"]');
    let KhoanMuc = $('#form-phieu-thu select[name="pt-KhoanMuc"]');
    let DienGiai = $('#form-phieu-thu textarea[name="pt-DienGiai"]');

    //Datatable Phiếu Thu
    let tbPhieuThu_filterValues = {};
    let fdate = null;
    let tdate = null;

    $('#table_phieu_thu thead tr').clone(true).appendTo('#table_phieu_thu thead');
    $('#table_phieu_thu thead tr:eq(1) th').each(function (i) {
        var title = $(this).text();
        if (i == 0) {
            return;
        }
        else if (i == 3) {
            $(this).html('<input class="search-date" style="width:100%" placeholder="Search ' + title.trim() + '" data-search-phieu-thu="' + i + '" /> ');
        }
        else {
            $(this).html('<input type="text" style="width:100%" placeholder="Search ' + title.trim() + '" data-search-phieu-thu="' + i + '"/>');
        }
    });

    var tbPhieuThu = $('#table_phieu_thu').DataTable({
        serverSide: true,
        bFilter: false,
        bInfo: false,
        ajax: function (data, callback, settings) {
            tbPhieuThu_filterValues.draw = data.draw;
            tbPhieuThu_filterValues.search = data.search["value"];
            tbPhieuThu_filterValues.fdate = fdate;
            tbPhieuThu_filterValues.tdate = tdate;
            tbPhieuThu_filterValues.start = data.start;
            tbPhieuThu_filterValues.length = data.length;
            tbPhieuThu_filterValues.order = data.order[0].column;
            tbPhieuThu_filterValues.dir = data.order[0].dir;

            tbPhieuThu_filterValues.search1 = $('input[data-search-phieu-thu=1]').val();
            tbPhieuThu_filterValues.search2 = $('input[data-search-phieu-thu=2]').val();
            tbPhieuThu_filterValues.search3 = $('input[data-search-phieu-thu=3]').val();
            tbPhieuThu_filterValues.search4 = $('input[data-search-phieu-thu=4]').val();
            tbPhieuThu_filterValues.search5 = $('input[data-search-phieu-thu=5]').val();
            tbPhieuThu_filterValues.search6 = $('input[data-search-phieu-thu=6]').val();
            tbPhieuThu_filterValues.search7 = $('input[data-search-phieu-thu=7]').val();

            $.ajax({
                url: '/PhieuThu/LoadPhieuThu',
                method: 'GET',
                data: tbPhieuThu_filterValues,
                success: function (msg) {
                    if (msg.status == 2) {
                        ...
                        if (tbPhieuThu_filterValues.draw != 1) {
                            toast.create({
                                title: 'Notification!',
                                text: msg.message,
                                icon: 'error_outline',
        ...
        fnCreatedRow: function (nRow, data, iDataIndex) {
            $(nRow).attr('data-id', data.THUID);
            $($(nRow).children()[3]).html(moment(data.NGAY).format('DD/MM/yyyy'));
            $($(nRow).children()[5]).html(convertCurrency(parseFloat(data.THU).toFixed(0)));
        },
        footerCallback: function (row, data, start, end, display) {
            ...
            if (data.length > 0) {
                var tt = Math.round(data[0].TONGTIENALL);
    ...
    $(tbPhieuThu.table().container()).on('keyup change', 'thead input', function (e) {
        tbPhieuThu.draw();
    });

    //Search ngày -> ngày
    $('#btn-searchdate-phieuthu').click(function () {
        fdate = $('#date-phieuthu input[name="chooseDate_Form"]').val();
        tdate = $('#date-phieuthu input[name="chooseDate_To"]').val();
        tbPhieuThu.columns.adjust().draw();
    });

    //Click
    $('#table_phieu_thu tbody').on('click', 'tr', function () {
        $(this).addClass('selected');
        $('#table_phieu_thu tbody tr').not(this).removeClass('selected');
    });

    //DoubleClick
    $('#table_phieu_thu tbody').on('dblclick', 'tr', async function () {
        $(this).addClass('selected');
        $('#table_phieu_thu tbody tr').not(this).removeClass('selected');
        LoadPhieuThu();
    });
    ...
    //Insert
    $('#btn-them-phieuthu').click(function () {
        $('#form-phieu-thu input').each(function (index, e) {
            $(e).val('');
            SoTien.prop('disabled', false);
            SoTienVND.prop('disabled', false);
        });
        NHID.val('');
        $('#form-phieu-thu textarea').each(function (index, e) {
            $(e).val('');
        });
        TienTeUSD();
        CreateCode().then((rs) => {
        ...
        $('#radio-ngan-hang').css('display', 'none');
        $('#form-phieu-thu').modal();
    });

    //Update
    $('#btn-edit-phieuthu').click(function () {
        LoadPhieuThu();
    });

    //Xóa
    $('#btn-delete-phieuthu').on('click', function () {
        $.ajax({
            async: false,
            type: 'GET',
            url: '/PhieuThu/CheckDeletePhieuThu',
            success: function (res) {
                if (res.rs) {
                    if ($('#table_phieu_thu tbody tr.selected').attr('data-id') != undefined) {
                        let idPhieuThu = $('#table_phieu_thu tbody tr.selected').attr('data-id');
                        if (confirm('Bạn có muốn xóa phiếu thu này?')) {
                            $.ajax({
                                async: true,
                                method: 'GET',
                                url: '/PhieuThu/DeletePhieuThu?id=' + idPhieuThu,
                                success: function (msg) {
                                    if (msg.rs) {
                                        tbPhieuThu.ajax.reload();
                                        toast.create({
                                            title: 'Notification!',
                                            text: 'Thành công',
                                            icon: 'check',
    ...
    //Nạp
    $('#btn-nap-phieuthu').on('click', function () {
        tbPhieuThu.draw();
    });

    //Save
    $('#btn-save-phieuthu').click(function () {
        let $currentForm = $('#form-phieu-thu');
        ...
        $.ajax({
            async: false,
            type: 'POST',
            url: '/PhieuThu/InsertUpdatePhieuThu',
            data: data,
            contentType: false,
            processData: false,
            success: function (msg) {
    ...
    //Excel
    $('#btn-export-phieuthu').on('click', function () {
        $.ajax({
            async: false,
            type: 'GET',
            url: '/PhieuThu/CheckExcelPhieuThu',
            success: function (msg) {
                if (msg.rs) {
                    var filterReport = {};
                    filterReport.draw = tbPhieuThu_filterValues.draw;
                    filterReport.search = tbPhieuThu_filterValues.search;
                    filterReport.start = tbPhieuThu_filterValues.start;
                    filterReport.length = tbPhieuThu_filterValues.length;
                    filterReport.order = tbPhieuThu_filterValues.order;
                    filterReport.dir = tbPhieuThu_filterValues.dir;
                    filterReport.fdate = tbPhieuThu_filterValues.fdate;
                    filterReport.tdate = tbPhieuThu_filterValues.tdate;

                    filterReport.search1 = tbPhieuThu_filterValues.search1;
                    filterReport.search2 = tbPhieuThu_filterValues.search2;
                    filterReport.search3 = tbPhieuThu_filterValues.search3;
                    filterReport.search4 = tbPhieuThu_filterValues.search4;
                    filterReport.search5 = tbPhieuThu_filterValues.search5;
                    filterReport.search6 = tbPhieuThu_filterValues.search6;
                    filterReport.search7 = tbPhieuThu_filterValues.search7;

                    filterReport.export = 1;
                    var link = `/PhieuThu/LoadPhieuThu?draw=` + filterReport.draw +
    ...
    //Print
    $('#btn-print-phieuthu').on('click', function () {
        $.ajax({
            async: false,
            type: 'GET',
            url: '/PhieuThu/CheckPrintPhieuThu',
            success: function (msg) {
                if (msg.rs) {
                    let id = $('#table_phieu_thu tbody tr.selected').attr('data-id');
                    if (id != undefined) {
                        var link = `/PhieuThu/Print?id=` + id;
                        window.open(link);
                    }
                    else {
                        toast.create({
    ...
    //$('#form-phieu-thu').on('hidden.bs.modal', function (e) {
    //    SoPhieu.prop('disabled', false);
    //    SRID.prop('disabled', false);
    //    NguoiThu.prop('disabled', false);
    //})
    //Function Load Phiếu Thu
    function LoadPhieuThu() {
        let id = $('#table_phieu_thu tbody tr.selected').attr('data-id');
        if (id != undefined) {
            LoadDetail(id).then((rs) => {
                ID.val(rs.data[0].THUID); //ID
                TienTeUSD(); //Tỉ giá
                SoPhieu.val(rs.data[0].THUCODE);  // số phiếu
                ...
                SoTien.val(rs.data[0].FCTHU); //số tiền FC (usd)
                SoTienVND.val(rs.data[0].THU1); //số tiền vnđ
                ...
                LyDo.val(rs.data[0].LDTID); //lý do
                ...
                $('#form-phieu-thu').removeClass('was-validated');
                $('#form-phieu-thu').modal();
            });
        } else {
            toast.create({
    ...
    ////function Load Chi Tiết Phiếu Thu
    async function LoadDetail(THUID) {
        return $.ajax({
            async: true,
            method: 'GET',
            url: '/PhieuThu/LoadDetail?THUID=' + THUID,
            success: function (msg) {
                return msg.data;
            }
        })
    ...
    async function CreateCode() {
        return $.ajax({
            async: true,
            method: 'GET',
            url: '/PhieuThu/CreateCodePhieuThu',
            success: function (msg) {
                return msg;
            },
        });
